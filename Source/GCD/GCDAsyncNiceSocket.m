...
#ifndef GCDAsyncNiceSocketLoggingEnabled
#define GCDAsyncNiceSocketLoggingEnabled 0
#endif

#if GCDAsyncNiceSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext GCDAsyncNiceSocketLoggingContext
...
#ifndef GCDAsyncNiceSocketLogLevel
#define GCDAsyncNiceSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = GCDAsyncNiceSocketLogLevel;
...
NSString *const GCDAsyncNiceSocketException = @"GCDAsyncNiceSocketException";
NSString *const GCDAsyncNiceSocketErrorDomain = @"GCDAsyncNiceSocketErrorDomain";

NSString *const GCDAsyncNiceSocketQueueName = @"GCDAsyncNiceSocket";
NSString *const GCDAsyncNiceSocketThreadName = @"GCDAsyncNiceSocket-CFStream";

NSString *const GCDAsyncNiceSocketManuallyEvaluateTrust = @"GCDAsyncNiceSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const GCDAsyncNiceSocketUseCFStreamForTLS = @"GCDAsyncNiceSocketUseCFStreamForTLS";
#endif
NSString *const GCDAsyncNiceSocketSSLPeerID = @"GCDAsyncNiceSocketSSLPeerID";
NSString *const GCDAsyncNiceSocketSSLProtocolVersionMin = @"GCDAsyncNiceSocketSSLProtocolVersionMin";
NSString *const GCDAsyncNiceSocketSSLProtocolVersionMax = @"GCDAsyncNiceSocketSSLProtocolVersionMax";
NSString *const GCDAsyncNiceSocketSSLSessionOptionFalseStart = @"GCDAsyncNiceSocketSSLSessionOptionFalseStart";
NSString *const GCDAsyncNiceSocketSSLSessionOptionSendOneByteRecord = @"GCDAsyncNiceSocketSSLSessionOptionSendOneByteRecord";
NSString *const GCDAsyncNiceSocketSSLCipherSuites = @"GCDAsyncNiceSocketSSLCipherSuites";
NSString *const GCDAsyncNiceSocketSSLALPN = @"GCDAsyncNiceSocketSSLALPN";
#if !TARGET_OS_IPHONE
NSString *const GCDAsyncNiceSocketSSLDiffieHellmanParameters = @"GCDAsyncNiceSocketSSLDiffieHellmanParameters";
#endif

enum GCDAsyncNiceSocketFlags
{
...
@interface GCDAsyncNiceSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation GCDAsyncNiceSocketPreBuffer
...
- (NSUInteger)readLengthForTermWithPreBuffer:(GCDAsyncNiceSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;
...
@implementation GCDAsyncNiceSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<GCDAsyncNiceSocketDelegate> delegate;
	...
	GCDAsyncNiceSocketPreBuffer *preBuffer;
	...
	GCDAsyncNiceSocketPreBuffer *sslPreBuffer;
...
- (instancetype)initWithDelegate:(id<GCDAsyncNiceSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	return [self initWithDelegate:aDelegate delegateQueue:dq socketQueue:NULL];
}

- (instancetype)initWithDelegate:(id<GCDAsyncNiceSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([GCDAsyncNiceSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[GCDAsyncNiceSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<GCDAsyncNiceSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<GCDAsyncNiceSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  GCDAsyncNiceSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncNiceSocketOtherError",
                                                           @"GCDAsyncNiceSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)setDelegate:(id<GCDAsyncNiceSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<GCDAsyncNiceSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id<GCDAsyncNiceSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id<GCDAsyncNiceSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<GCDAsyncNiceSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak GCDAsyncNiceSocket *weakSelf = self;
			
            dispatch_source_set_event_handler(self->accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong GCDAsyncNiceSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		__strong id<GCDAsyncNiceSocketDelegate> theDelegate = delegate;
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create GCDAsyncNiceSocket instance for accepted socket
			
			GCDAsyncNiceSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncNiceSocketConnectTimeoutError",
	                                                     @"GCDAsyncNiceSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncNiceSocketReadMaxedOutError",
														 @"GCDAsyncNiceSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncNiceSocketReadTimeoutError",
	                                                     @"GCDAsyncNiceSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncNiceSocketWriteTimeoutError",
	                                                     @"GCDAsyncNiceSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncNiceSocketClosedError",
	                                                     @"GCDAsyncNiceSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:GCDAsyncNiceSocketErrorDomain code:GCDAsyncNiceSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the GCDAsyncNiceSocketUseCFStreamForTLS flag.
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via GCDAsyncNiceSocketUseCFStreamForTLS)
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			...
			NSNumber *value = [tlsSettings objectForKey:GCDAsyncNiceSocketUseCFStreamForTLS];
...
{
	GCDAsyncNiceSocket *asyncSocket = (__bridge GCDAsyncNiceSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	NSNumber *shouldManuallyEvaluateTrust = [tlsSettings objectForKey:GCDAsyncNiceSocketManuallyEvaluateTrust];
	...
	//  3. GCDAsyncNiceSocketSSLPeerID
	//  4. GCDAsyncNiceSocketSSLProtocolVersionMin
	//  5. GCDAsyncNiceSocketSSLProtocolVersionMax
	//  6. GCDAsyncNiceSocketSSLSessionOptionFalseStart
	//  7. GCDAsyncNiceSocketSSLSessionOptionSendOneByteRecord
	//  8. GCDAsyncNiceSocketSSLCipherSuites
	//  9. GCDAsyncNiceSocketSSLDiffieHellmanParameters (Mac)
    // 10. GCDAsyncNiceSocketSSLALPN
	//
	...
	value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLPeerID."]];
		return;
	}
	
	// 4. GCDAsyncNiceSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. GCDAsyncNiceSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. GCDAsyncNiceSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. GCDAsyncNiceSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. GCDAsyncNiceSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. GCDAsyncNiceSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif

    // 10. kCFStreamSSLCertificates
    value = [tlsSettings objectForKey:GCDAsyncNiceSocketSSLALPN];
    if ([value isKindOfClass:[NSArray class]])
    {
        ...
        {
            NSAssert(NO, @"Security option unavailable - GCDAsyncNiceSocketSSLALPN"
                     @" - iOS 11.0, macOS 10.13 required");
            [self closeWithError:[self otherError:@"Security option unavailable - GCDAsyncNiceSocketSSLALPN"]];
        }
    }
    else if (value)
    {
        NSAssert(NO, @"Invalid value for GCDAsyncNiceSocketSSLALPN. Value must be of type NSArray.");
        
        [self closeWithError:[self otherError:@"Invalid value for GCDAsyncNiceSocketSSLALPN."]];
        return;
    }
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use GCDAsyncNiceSocketSSLProtocolVersionMin & GCDAsyncNiceSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[GCDAsyncNiceSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"GCDAsyncNiceSocketManuallyEvaluateTrust specified in tlsSettings,"
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("GCDAsyncNiceSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:GCDAsyncNiceSocketThreadName];
...
+ (void)scheduleCFStreams:(GCDAsyncNiceSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(GCDAsyncNiceSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	GCDAsyncNiceSocket *asyncSocket = (__bridge GCDAsyncNiceSocket *)pInfo;
